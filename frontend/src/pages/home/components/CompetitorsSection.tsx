          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-5 lg:grid-cols-3">
            {competitors.map((c) => (
              <Link
                ...
                className="flex flex-col rounded-2xl border border-background-200/70 bg-background-50 p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg"
              >
                ...
                <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
                  <div>
                    <p className="text-xs text-foreground-400">Trustpilot TrustScore</p>
                    <p className="text-sm font-semibold text-foreground-900">
                      {c.trustpilot_rating != null ? `${c.trustpilot_rating.toFixed(1)} out of 5` : "Unavailable"}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-foreground-400">Apprenticeship achievement rate</p>
                    <p className="text-sm font-semibold text-foreground-900">{formatPercent(c.fatp_achievement_rate)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-foreground-400">Ofsted report</p>
